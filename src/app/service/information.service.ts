...
export class InformationService {
  ...
  private async isOpenEstablishment(): Promise<boolean> {
    ...
    const week = await this.establishmentCloseInNextDay(this.information);
  ...
  private async establishmentCloseInNextDay(information: Information): Promise<Week> {
    ...
    const yesterdayWeekDay = today == 0 ? 6 : today - 1;
    const yesterdayWorkInformation: Week = await information.week_work.filter(value => yesterdayWeekDay == value.day)[0];
  ...
  private async isRecentVerification(){
    const lastInfoUpdate = await this.storage.get(DBConstants.INFO_LAST_UPDATE);
